...
arcpy.management.MakeFeatureLayer(os.path.join(path, "USALandmarks.lpk"), "golf_temp", "FEATTYPE = 'Golf Course Ground'")  # Golf courses
...
arcpy.conversion.FeatureClassToGeodatabase("airports_temp", path)

## Remove extraneous columns   
arcpy.management.DeleteField('airports_temp2', "MNFC;LOC_ID_1")

## Remove airport grounds
arcpy.management.EliminatePolygonPart("airports_temp2", os.path.join(path, "airports_temp3"), "PERCENT", "0 SquareMeters", 50, "CONTAINED_ONLY")  # absorbs polygon parts w/in runways that are < 50% total area
arcpy.management.MakeFeatureLayer('airports_temp3', 'airports_temp4', "FEATTYPE = 'Airport runway' AND STATE IS NOT NULL")  # also remove NULL values

## Create buffer of 100m
arcpy.analysis.Buffer("airports_temp4", os.path.join(output_gdb, "airports_buff100"), "100 Meters", "FULL", "ROUND", "LIST", "NAME;LOC_ID;STATE;ACT_DATE", "PLANAR")
...
# ^ Error in Python notebook --> works manually using Dissolve tool using these entries
...
## Create buffer of 50m
arcpy.analysis.Buffer("rail", os.path.join(output_gdb, "rail_buff50"), "50 Meters", "FULL", "ROUND", "LIST", "STATEAB;YARDNAME", "PLANAR")
