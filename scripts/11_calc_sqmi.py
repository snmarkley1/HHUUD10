...
erase_features = ["cemeteries", "industrial_areas", "parks", "rail_buff50", "water", "golf40", "airports40"] # remove t10_zero
...
t10_list = []
for m in aprx.listMaps():
    for lyr in m.listLayers("*0") + m.listLayers("*9"):
        t10_list.append(lyr)
        
del_fields = ["pdev92", "pdev92_dhi", "pdev01", "pdev01_dhi", "pdev11", "pdev11_dhi", "sqmi40"]
for i in t10_list:
    arcpy.management.DeleteField(i, del_fields)

## Join t10_prep to others
arcpy.management.MakeFeatureLayer("t10", "t10")
for i in ["t10_prep"] + t10_list:
  arcpy.management.AddJoin('t10', "GISJOIN", i, "GISJOIN", 'KEEP_ALL')

## Save out
arcpy.conversion.FeatureClassToFeatureClass("t10", env.workspace, "t10_join")

## Clear display
for m in aprx.listMaps():
    for lyr in m.listLayers("t10_prep*") + m.listLayers("t10"):
        m.removeLayer(lyr)
